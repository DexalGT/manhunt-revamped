...
# ── Start lead phase ─────────────────────────────────────────────────────────
scoreboard players set $lead_timer mh_display 45

scoreboard players set $state mh_enabled 1
scoreboard players set $ticks mh_ticks 0

title @a times 0 80 20
title @a title {"text":"MANHUNT","bold":true,"color":"dark_red"}
title @a subtitle {"text":"Hunters blinded — 45 seconds head start","color":"yellow"}
